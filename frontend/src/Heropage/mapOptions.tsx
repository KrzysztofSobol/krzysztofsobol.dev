...
function MapOptions({onGenerateMap} : mapOptionProps) {
    ...
    const toggleOptions = () => {
        if (!isOpen) {
            setRotationClass('rotate-clockwise');
        } else {
            setRotationClass('rotate-counterclockwise');
        }
    ...
    return(
        <>
            <svg className={`settings-button ${rotationClass}`} onClick={toggleOptions} width="70px" height="70px" viewBox="0 0 24 24" fill="none"
